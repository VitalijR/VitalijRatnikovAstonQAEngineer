...
// В классе PaySectionPage собраны локаторы и методы для тестирования блока оплаты и фрейма
public class PaySectionPage {
    ...
    public String [] getPlaceholderPhoneNumber(int index, String[] expected) {
        ...
        switch (index){
            case 0: WebElement number = driver.findElement(By.id("connection-phone"));
                s[0] = number.getAttribute("placeholder");
                s[1] = expected [index];
                break;
            case 1: WebElement homeInternet = driver.findElement(By.id("internet-phone"));
                s[0] = homeInternet.getAttribute("placeholder");
                s[1] = expected [index];
                break;
            case 2: WebElement installmentPlan = driver.findElement(By.id("score-instalment"));
                s[0] = installmentPlan.getAttribute("placeholder");
                s[1] = expected [index];
                break;
            case 3: WebElement debt = driver.findElement(By.id("score-arrears"));
    ...
    public void enterPhoneAndSum(String phone, String sum) {
        WebDriverWait wait = new WebDriverWait(driver, Duration.ofSeconds(10));
        wait.until(ExpectedConditions.visibilityOf(driver.findElement(By.id("connection-phone"))));
